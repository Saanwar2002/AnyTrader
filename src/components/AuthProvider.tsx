...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  useEffect(() => {
    ...
    const authUnsubscribe = onAuthStateChanged(auth, async (firebaseUser) => {
      ...
      // Exception: Allow the super-admin email to bypass verification check (to fix login issues)
      ...
      if (firebaseUser) {
        ...
        profileUnsubscribe = onSnapshot(doc(db, "users", firebaseUser.uid), async (docSnap) => {
          if (docSnap.exists()) {
            ...
            if (firebaseUser.email?.toLowerCase() === "saanwar2002@gmail.com") {
              setProfile({ role: "admin", name: firebaseUser.displayName || "Admin" });
            } else {
              setProfile(null);
            }
          }

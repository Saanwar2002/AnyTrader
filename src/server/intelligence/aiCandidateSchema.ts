...
export const AIExtractionCandidateSchema = z.object({
  ...
  candidateConfidence: z.number().min(0).max(1).optional(),
}).strict().superRefine((candidate, ctx) => {
  // Ensure at least one observation, inference, problem, intervention, outcome, or condition exists
  const hasSubItem =
  ...
  if (!hasSubItem) {
    ctx.addIssue({
      ...
      message: 'AI candidate extraction must contain at least one observation, inference, problem, intervention, outcome, or condition',
      path: ['observations'],
    });
  }
});

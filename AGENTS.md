...
## Active Task
- **Completed**: Task 28 — Evidence & Data Provenance Graph implemented and verified. Created `ProvenanceGraphService` (`provenanceGraph.ts`) enforcing canonical tenant model (`tenantId === request.auth.uid`), 6-stage lineage lifecycle (`Source -> Evidence -> Observation -> Extraction -> Derived Intelligence -> Projection`), controlled 7 node types and 9 relation types, deterministic SHA-256 ID and content hashing, zero-bypass AI security boundary, internal AI Bot preservation (`internal_ai_use: 'allowed'`), independent rights purposes, bounded graph traversals, and append-only immutability. Verified against canonical commit `abac92662cab4cc7352de4f9f9d2e2419aad9c29` (452/452 emulator security tests passing across 13 files, 646/646 ordinary tests passing across 41 files, lint clean, build clean, 0 critical audit failures). Documented in `/TASK_28_EVIDENCE_PROVENANCE_GRAPH_REPORT.md` and `DEVELOPMENT.md`. Task 29 has NOT started.

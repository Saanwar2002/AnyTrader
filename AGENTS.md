...
## Active Task
- **Completed**: Task 31 — AI Training & Usage Controls implemented and verified. Created `AiTrainingUsageControlsService` (`src/server/intelligence/aiTrainingUsageControls.ts`) establishing server-authoritative controls over AI usage purposes and training consent. Enforced explicit purpose isolation (`internal_ai_use != external_ai_training`), explicit opt-in training consent requirement for external AI model training, conservative evaluation (`unknown != allowed`), integration with Task 27 Data Rights, Task 28 Provenance Graph, Task 29 Contractor Archive Rights, and Task 30 Data Classification Eligibility. Verified across unit tests and Firebase emulator integration security rules. Documented in `DEVELOPMENT.md` and `/TASK_31_AI_TRAINING_USAGE_CONTROLS_REPORT.md`. Task 32 has NOT started.

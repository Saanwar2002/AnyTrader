...
- 2026-06-01: Engineered a high-fidelity interactive Surge Overlay Badge for the predictive heatmap overlays on the Driver's map. Each badge center renders a glass-morphic HUD card with a thin white border containing an animated pulsing status dot matching the intensity tier (Red for High Surge, Orange/Amber for Medium Surge, Blue for Low Surge). The card displays the precise zone label, the active dynamic rate (e.g., `+£3.50` or `1.6x`), and dynamic subtitles explaining the exact trigger conditions (e.g., `> 20m Wait or > 3:1 Demand`). This ensures drivers know exactly which areas are surging and at what intensity levels, even with low physical circle opacities or while active directions are rendering.
...
- 2026-05-31: Resolved a persistent flickering bug on the Driver Terminal's heads-up navigation overlay. Under frequent route GPS simulations and mapCenter rendering updates, the `<motion.div>` element inside `<AnimatePresence>` lacked a stable React `key`. Instantiated a static, persistent `key="heads-up-navigation"` on the motion container to inform React Reconciler and Framer Motion of its permanent tree footprint. This safely suppresses unnecessary enter/exit layout animations, rendering real-time instruction and distance updates in an exceptionally fluid, flicker-free manner with full backward stylistic compliance preserved.
- 2026-05-31: Refactored the Driver Terminal's heads-up navigation overview flow to support manual toggling. Removed the automatic 3-second timeout inside the `useEffect` trigger that forced overview mode back into active head-up navigation. This empowers drivers to switch freely between active heads-up tracking and standard route overview, manually turning head-up view back on only when the green compass/orientation button is kept toggled (pressed) active.
...
- **V8.1 Task 12A — AI Lineage Bypass Removal & Platform-Wide Context Hardening**:
  - **Complete Removal of `skipLineageCheck`**: Removed `skipLineageCheck` from `PipelineOptions` and `processAICandidateToCanonical` (`src/server/intelligence/aiCandidateBoundary.ts`). Enforced that there is NO production runtime option allowing callers to bypass evidence lineage validation. If `firestoreDb` is provided, evidence lineage check ALWAYS executes.
  ...
  - **Comprehensive Unit & Emulator Test Matrix (`tests/unit/task12AIOutputSecurityBoundary.test.ts`, `tests/unit/firebaseEmulatorIntelligenceV81.test.ts`)**: 
    - 18 unit tests in `task12AIOutputSecurityBoundary.test.ts` (+2 new regression tests) proving: callers cannot pass legacy bypass flags to skip lineage validation when evidence is missing (hard rejection), and platform-wide aggregate types (`contractor`, `material`, `project`, `customer_request`) attach server metadata, enforce lineage checks, and canonicalize safely.
    - Added real Firestore emulator tests in `firebaseEmulatorIntelligenceV81.test.ts` verifying that bypass attempts are rejected and platform-wide aggregate extractions persist correctly to `intelligence_extractions`.
  - **Full Test Matrix**: 100% test pass rate across all 28 unit test suites (397/397 passing tests), clean linter (`npm run lint`), and successful application build (`compile_applet`).

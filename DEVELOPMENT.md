...
- 2026-06-02: Upgraded the AnyRoller Admin "Trust & Fairness Engine" (`PrioritySettings.tsx`) configuration panel to synchronize directly with the active `/platform_config/rides` collection via real-time Firestore triggers and standard setDoc merge queries. This unifies operational parameter controls (Newcomer Boosts, Rating Priorities, Serial Canceller Protection thresholds, and Fair Distribution policies) under the same centralized document contract as the Dispatch Engine settings, completely eliminating configuration conflicts and ensuring a single cohesive pool of true system parameters.
- 2026-06-02: Revamped all system toggle switches (such as those in `DispatchEngine.tsx`, `ZonesGeofences.tsx`, and `PrioritySettings.tsx`) to glow brand-specific emerald green (`bg-[#00D26A]`) when active, achieving absolute visual consistency. Additionally, engineered a compact **Platform Core Controls** action header directly below the Admin Panel header search input row. It mounts two real-time responsive toggles for **Auto Dispatch** and **Auto Surge**, secured behind a rigid double-confirm dialog (white glossy card styled with a thin black border) that forces administrators to re-confirm toggle requests before updating globally active Firestore configurations.
...
- 2026-05-30: Upgraded Driver Terminal and Passenger Booking maps to leverage Google Maps JavaScript API high-performance WebGL Vector Maps. Configured the cloud-linked Map ID (`8d7f862551b8bb49989453ed`) on all `premiumMapOptions` definitions (including Admin's LiveMap). Enabled completely fluid camera rotations (`mapInstance.setHeading(heading)`) natively matching the driver's device bearing during heads-up active navigation overview mode (when the green button is pressed). Removed complex and fragile absolute container rotation hacks/offsets on the viewport wrapper div and custom marker overlays (`OverlayViewF`), resolving the sliding viewport bug, restoring the upright position of the driver's vehicle marker, and restoring upright legibility of all street and road names during dynamic map rotations.
- 2026-05-30: Solved and persisted user-preferred map zoom levels across multiple screens. In `PassengerBooking.tsx`, the default pre-booking zoom has been restored to previous standard levels (17). If the passenger manually zooms pre-booking, the preference is saved in `localStorage` under `passenger_preferred_booking_zoom` and restored upon subsequent entries. During active travel, if the driver approaches (<400m) or enters the `"arrived"` status, the passenger's map automatically shifts to street view level (18 or 19) to show exactly where the driver is parked. Clapped camera fitBounds overrides below 18 to avoid erratic zooming. In the `DriverTerminal.tsx` navigation panel, the initial map zoom similarly loads from `driver_preferred_zoom` in local storage (defaulting to 17) and stores manual zoom changes. In the Master Admin's `LiveMap.tsx` dashboard, resolved the annoying real-time query re-render reset zoom bug by binding map zoom to are React state variable (`mapZoom`) initialized and persisted directly inside local storage under `admin_live_map_zoom`.
- 2026-05-30: Resolved a subtle map zoom corruption issue across passenger and driver views. In `PassengerBooking.tsx`, when calculating route distances and drawing poly lines, maps automatic boundaries fitting (`fitBounds`) zooms out the view (e.g. to level 13 or 14). Because the passenger was in the `"details"` phase, `onZoomChanged` was writing this automatic wide zoom back into `localStorage` as their default preference. Fixed by gating `localStorage` writes with a check that both pickup and dropoff coords are not simultaneously set (`!pickupCoords || !dropoffCoords`). In `DriverTerminal.tsx`, similarly restricted writing `driver_preferred_zoom` to when there is no active ride (`!activeRide?.id`) to prevent active trip dynamic zooms from overwriting the driver's default waiting map zoom level.